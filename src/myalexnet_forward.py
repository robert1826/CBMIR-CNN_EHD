...
def gen_batch(index, imgs, batch_size):
	...
	while(index < len(imgs)):
		...
		name = re.search('(\d+)(?=(\.png|\.jpg))', i).group(1)
		# name = i.split('/')[1]
...
if __name__ == '__main__':
	if len(sys.argv) < 4:
		print('error : no input files. (dataset file root, dataset.txt, labels pickle file)')
		exit(1)
	...
	dict = unpickle(pickle_file)['labels']
	...
	with tf.Session(graph=graph) as session:
		...
		while(index+batch_size<len(imgs)):
			if(index % 20 == 0):
				print('index :', index, 'from', len(imgs))
	...
	out_file = open(file_name + '_desc', 'wb')
	save = {
		'desc7': desc7,
	    'desc': desc8,
	    'labels': train_labels[:len(desc8)],
